...
// the schema). Prefers the direct OpenAI key (OPENAI_API_KEY); falls back to the
// Vercel AI Gateway (AI_GATEWAY_API_KEY, model prefixed "openai/…"), mirroring the
// provider selection in app/api/copilotkit/route.ts. No new dependency — reuses the
// `openai` SDK already in the project.
// ─────────────────────────────────────────────────────────────────────────────
