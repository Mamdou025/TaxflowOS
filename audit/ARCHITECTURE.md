...
| `POST /api/copilotkit` | `app/api/copilotkit/route.ts` | **CopilotKit runtime** (CopilotRuntime + OpenAIAdapter) — powers the chat panel. **Prefers the Vercel AI Gateway** when `AI_GATEWAY_API_KEY` is set (OpenAI-compatible base URL `https://ai-gateway.vercel.sh/v1`; base `OPENAI_CHAT_MODEL` normalized to `provider/model` via `normalizeModelSpec`), else `OPENAI_API_KEY` direct. Middleware runs the intent gate + per-turn model tiering (overrides already go through the gateway) + orphan repair |
| `POST /api/agent-lab` | `app/api/agent-lab/route.ts` | **Agent Lab** — self-contained AI-SDK agent (`runAgent` in `features/agent-lab/agent.ts` → `generateText` + `stepCountIs` loop). Configurable model/system/tools/temperature/steps; **attached `documents`** (extracted client-side; Excel via in-browser `xlsx`, the rest via `/api/assistant/extract`) are injected into the system prompt (capped ~90k chars) in `docMode:'full'`, or — in `docMode:'retrieval'` — NOT injected, with a per-request doc-bound `searchDocuments` (keyword RAG) tool instead; reported back as `docContext`. Returns text + per-step tool calls + usage. Model id resolves to direct OpenAI (`OPENAI_API_KEY`) or a gateway string (`AI_GATEWAY_API_KEY`). A **model router** (`features/agent-lab/model-router.ts`, pure `planForModel`) sets per-model provider options: **omits `temperature`** for the Anthropic 4.7+/5-era family (Opus 4.7/4.8, Sonnet 5, Fable 5 — they 400 on non-default sampling), sends Anthropic **`effort`** (`providerOptions.anthropic.effort`) only where supported (not Haiku 4.5 / Sonnet 4.5), and **prompt-caches** the system/doc block (`providerOptions.anthropic.cacheControl: ephemeral` on a system message) so repeated runs read it at ~10% price. Selecting **`sina-auto`** (`AUTO_MODEL_ID`) instead of a model runs `routeAuto()` — a deterministic keyword classifier over the last user message → Haiku (quick) / Sonnet 5 (balanced/reasoning) / Opus 4.8 (deep) with a suggested effort (a manual effort overrides). The response carries `applied` (model/auto/tier/reason/provider/effort/cache/temperatureSent) + `usage.cachedInputTokens`. **Context scope** (client-side, `features/agent-lab/context-router.ts`): with the Advanced **Auto** toggle, `selectContext()` picks per-turn which prompt folders + tools are relevant to the question (deterministic keyword match; always keeps the identity folder; falls back to all when nothing matches) — so the route receives a **scoped** `system` + `enabledTools` subset instead of the full set. `full` mode sends everything (unchanged). **Fiscal mode** (client-side, `features/agent-lab/fiscal.ts`, `fiscalPreamble`): when on, a pinned fiscal context (tax year/jurisdiction/entity/residency/currency) + 5 non-negotiable rules (cite authority · no self-computed figures · currency-of-law · defer on judgment · flag uncertainty) is **prepended after scoping** to the `system` the route receives — always present, never pruned. All three (model router, context scope, fiscal mode) are client-driven; the route itself is unchanged beyond accepting the resulting `system`/`enabledTools`/`effort`. Auth-gated in production. **Independent of `/api/copilotkit`** |
...
> **[UPDATED 2026-07-24 — Phase 1: unified into one "Sina"]** The four specialist *personas* (Sofi/Théo/Mira/Nova) are being collapsed into a single agent, **Sina**. The per-turn injection no longer swaps identity — `features/assistant/runtime/agents/sina.ts` `sinaTurnDirective(route)` now injects a **"DOMAIN FOCUS FOR THIS TURN"** note (the workflow's domain expertise, still sourced from `agents/specialists.ts`) while Sina stays Sina. `app/api/copilotkit/route.ts` calls `sinaTurnDirective` instead of `selectSpecialist`+`specialistDirective`; `INSTRUCTIONS()` opens with "You are Sina…" and its former SPECIALISTS section is now "YOUR EXPERTISE — one unified specialist". `specialists.ts` is retained as the **domain-knowledge registry** (`selectSpecialist`/`EXPERTISE` still feed the domain text + the routing eval). **Phase 2 core (done 2026-07-24, tsc clean):** the pervasive faces are unified — a single `SINA` coworker (`lib/coworkers.ts`); `coworkerForMessage` attributes every reply to Sina (keeping the truthful non-agent actors: UI Composer / UI Concierge / deterministic Workflow Engine); per-message specialist map retired (`message-specialists.tsx` → empty, falls back to `coworkerForMessage`); the presence chip (`specialist-presence.tsx`, renamed intent → shows "Sina · Reviewing…"). **Phase 2 tail (done 2026-07-24, tsc clean):** the empty-state **"Talk to an agent" hero is now task quick-starts** (`HeroLaunchpad` in `assistant-thread.tsx` maps `WORKFLOWS` (ready) → `launchStartWorkflow` cards: Calculate FAPI · Roulement · Expense · Campaign); the **address-an-agent flow is removed** — the composer's 👥 agents popover (`aside-thread.tsx`), the `@`-mention agent commands + the agent entries in the composer tool list (`use-assistant.tsx`) are gone (an "agent" search hit now starts that domain's workflow via `launchStartAgent`). The unreachable `addressedAgent` chip/greeting plumbing is left inert (guarded by `addressedAgent &&`, never set). `ASSISTANT_SPECIALISTS` still gates the domain-focus injection. **Phase 3 (config seam, done 2026-07-24, tsc clean):** `features/assistant/runtime/agent-config.ts` — a persisted `LiveAgentConfig` (`agentConfigAtom`, `atomWithStorage` key `sina-agent-config`: `{fiscalMode, fiscal, extraInstructions, modelSpec, effort}`) + `applyLiveConfig(base, config)` which layers base identity → fiscal preamble (graduated from the Agent Lab's `fiscal.ts`) → operator additions. Wired into the live `<CopilotChat instructions={applyLiveConfig(INSTRUCTIONS(), agentConfig)}>` (`assistant-thread.tsx`), so the Build tab's fiscal/extra-instructions govern the real Sina. **Dormant by default** (config off/empty) until the Build UI writes it. `modelSpec`+`effort` are stored/surfaced but NOT yet driving the live model (needs a verified CopilotKit client→`forwardedProps` path — flagged follow-up). **Phase 4 (Agent page, done 2026-07-24, tsc clean):** a new workspace surface for the one live agent at `/agent` (`features/agent-hub/`: `agent-page.tsx` = `AgentPage` with `usePageMenu('agent')` Overview·Build tabs + `agentTabAtom`; `agent-hub.tsx` thin export; `app/agent/page.tsx` opens the workspace window via `openWorkspaceWindowAtom({pageKey:'agent'})`; registered as `RESOURCES` id `agent` + an **Agent** `NAV_LINKS` pill). **Overview** introspects the live agent (identity/domains, prompt layers, tools, context-it-sees, models/routing, fiscal preamble preview). **Build** writes `agentConfigAtom` (Phase 3 seam) — fiscal + operator instructions govern the live chat immediately; model/effort stored. **Net: Phases 1–4 complete — the live chat is one Sina (brain + face + config seam) with a tabbed Overview/Build page.** Remaining follow-up: wire the live model/effort override (CopilotKit client→`forwardedProps`, needs verification).

> **[FINALIZED 2026-07-24 — "finish the Sina collapse"]** The last four-persona residue is now removed (not just left inert), `tsc` clean, `pnpm assistant:evals` PASS. Concretely: the dead persona UI (`agent-roster.tsx`, `agent-builder.tsx`, `agent-greeting.tsx`) is **deleted**; `lib/agents.ts` is now an honest **workflow-domain catalog** (`Agent` slimmed to `{id,name,role,workflow?,live}`, entries renamed FAPI/Section 85 rollover/Expense/Campaign — no Sofi/Théo/Mira/Nova, no persona-render fields); `agents/specialists.ts` renamed its symbols to the domain vocabulary (`WorkflowDomain`/`WORKFLOW_DOMAINS`/`selectDomain`/`domainForWorkflow`; dead `specialistDirective` removed); `coworkerForAgent`/`coworkerForWorkflow` are **deleted** and every run/attribution now resolves to **`SINA`** (`workflow-run-flow.tsx`, `use-assistant.tsx`, `aside-thread.tsx`); the unreachable `addressedAgent` plumbing + the dormant scripted-"thinking" narration seam (`agent-thinking.tsx`, `thinkingCoworkerAtom`/`setThinkingCoworkerAtom`) are **removed**. The three obsolete persona e2e tests were dropped; the run-coworker test now asserts **Sina**.

One assistant, one conversation: **Sina**. A turn about a resolved workflow surfaces that workflow's DOMAIN FOCUS (expertise text), applied without any identity swap.

- **Selection** (`features/assistant/runtime/agents/specialists.ts`, pure/isomorphic): `selectDomain(route)` maps a resolved workflow target → its `WorkflowDomain` (label/role/`expertise`, keyed off `lib/agents.ts`); general/navigation → null.
- **Brain (server):** the `/api/copilotkit` middleware (same one as the gate + tiering) injects the domain expertise as a **context item** (`DOMAIN FOCUS FOR THIS TURN — You are Sina …`, `sinaTurnDirective`) using the route the gate already computed — reaches the model without touching the thread. Flag `ASSISTANT_SPECIALISTS` (default on).
- **Face (client):** `features/assistant/ui/specialist-presence.tsx` (`SpecialistPresence`, headless, mounted once in app-shell) sets the live `CoworkerActivity` indicator to **Sina** while a reply streams. Purely COSMETIC (writes only the presence atom), and yields to a live run (which owns the indicator).
- **Per-message avatars:** `coworkerForMessage()` attributes every plain reply + workflow proposal to **Sina**, keeping the truthful non-agent actors (UI Composer for `generateUI`, UI Concierge for page ops). `message-specialists.tsx` is retained as an always-empty override seam (falls back to `coworkerForMessage`).
