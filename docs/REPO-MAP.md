...
**Assistant / Chat** — the Ask/Propose/Execute intent-gated AI assistant. Pure deterministic runtime in `features/assistant/runtime/**` (routing, memory, model-tiering, evals; server-only, no React). Server endpoint: `app/api/copilotkit/route.ts` (CopilotKit `BuiltInAgent` + gate/orphan-repair middleware). Client entry: `features/assistant/workspace/copilot-workspace-panel.tsx` = the `/` route `ChatWorkspace` (`app/page.tsx`). The "client brain" is the god-hook `features/assistant/ui/use-assistant.tsx`. **WARNING:** chat-rendering files are scattered between `features/assistant/ui/**` and `features/assistant/workspace/**`.
...
**Platform spine (not a mini-app)** — DB/auth/API shared by everyone. Drizzle schema `platform/db/schema.ts` (11 tables) + client `platform/db/index.ts`; auth `platform/auth/auth.ts` + `components/auth/dialog.tsx`; 36 route handlers under `app/api/**`; browser client `platform/api-client.ts`; aspirational kernel vocabulary `shared/kernel/**` (types-only, only `ActorKind` consumed at runtime).
...
| **Make chat answer about a worksheet** | `features/worksheets/intel/**` (contract `types.ts`, generic `template-adapter.ts`, registry `registry.ts`) | Publisher `features/assistant/ui/worksheet-copilot.tsx` |
| **Change assistant behavior/routing** | Runtime `features/assistant/runtime/**` (routing `routing/classify.ts`, gate `routing/gate.ts`); server `app/api/copilotkit/route.ts` | Client actions in `features/assistant/ui/use-assistant.tsx` |
| **Change chat UI / thread** | `features/assistant/ui/assistant-thread.tsx` + `features/assistant/workspace/{aside-thread,thread-messages,workflow-run-flow}.tsx` | (chat rendering straddles both folders) |
...
| **Change DB schema / API** | `platform/db/schema.ts` + migrations; handlers under `app/api/**`; browser calls `platform/api-client.ts` | `platform/db/index.ts` omits `assistantMemories` from its query object (drift) |
...
| `features/assistant/ui/use-assistant.tsx` | 864 | God-hook client brain: ~15 actions + ~8 readables. |
| `features/assistant/workspace/workflow-run-flow.tsx` | 800 | Renders a run inline in chat (also `app/run/[workflowId]`). |
...
| `platform/api-client.ts` | 664 | Typed browser→API client (~17 UI importers). |
...
`features/workflow-builder/ui/workflow-studio-shell.tsx` (1426), `worksheet-page-menu.tsx` (201), `components/overlays/alert-overlay.tsx` (132), `inspector/mock-runs.ts` (123), `config/condition-config.tsx` (31), `components/{inscope-home,inscope-sidebar,ambient-orbs,canvas-page-wrapper}.tsx`, `features/assistant/ui/{coworker-activity,scope-launchpad}.tsx`, `features/assistant/workspace/field-editor.tsx`, `lib/integrations/vercel.ts` (671), `lib/utils/template.ts` (549), `features/assistant/runtime/{index,errors}.ts` + `routing/intent-router.ts`, `shared/workflow-engine/execution/runtime/{runner,validation}.ts`, `features/worksheets/legacy/pages/{Workbench,Home,NotFound,FapiWorksheet}.tsx`, `features/worksheets/legacy/components/{OrbitalStage,Map,ManusDialog,ErrorBoundary}.tsx`, `features/worksheets/legacy/hooks/useComposition.ts`, `app/copilot-test/`.
...
- `templates/next-project/**` — file-copy template for `app/api/workflows/[workflowId]/download/route.ts`.
...
- `shared/kernel/**` — intentional (not-yet-wired) migration scaffolding.
