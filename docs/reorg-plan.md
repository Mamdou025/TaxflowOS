...
**Phase 2 — Stand up the shared foundation (everything depends on it).**
3. **`shared/ui/`** — split domain widgets out of `components/ui/` (row A) **first**, then move primitives + `globals.css` split + `scope-orb` + `theme-toggle` + generic overlays (rows 3, 13).
4. **`shared/app-shell/`** + **`shared/stores/`** (rows 4, 5) — `app/layout.tsx` re-exports from `shared/app-shell`.
