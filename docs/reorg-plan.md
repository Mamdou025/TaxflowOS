...
**Phase 4 — Move the builder UI + execution onto the engine. ✅ DONE 2026-07-21** (modulo 2 integration widgets + 6 settings overlays intentionally deferred to Phase 7 platform).
6. ✅ **Step 4a (`0a81afa`)** — `components/workflow/**` (78) → `features/workflow-builder/ui/`; `components/ai-elements/**` (8) → `ui/ai-elements/`. **Fixed the excel-utils inversion**: `excel-utils.ts` → `shared/workflow-engine/parsing/`. Codemod + fixed cross-boundary relatives. tsc 0.
7. ✅ **Step 4b (`f49bf72`)** — `backend/**` (55) → **`shared/workflow-engine/execution/`** (deviation: engine's `local-tool-registry` imports the block runtime, so execution belongs with the engine, not a feature — avoids engine→feature inversion). Codemod + smoke-script relatives. tsc 0; discover-plugins clean.
8. ✅ **Step 4c (`98fdbb6`)** — the 3 template-variable widgets (`template-badge-input/-textarea`, `template-autocomplete`) → `features/workflow-builder/ui/inputs/` (they polluted `shared/ui` with engine-state imports). tsc 0. *(The 2 `integration-icon/-selector` widgets stay in `shared/ui` — cross-cutting; → platform Phase 7.)*
9. ✅ **Step 4d (`030a4b7`)** — split `components/overlays/`: generic framework (`overlay`, `overlay-provider/-container/-header/-footer/-sync`, `types`, `confirm-overlay`) → **`shared/ui/overlays/`**; builder overlays (`configuration`, `export-workflow`, `workflow-issues`, `make-public`) → **`features/workflow-builder/ui/overlays/`**. The 6 integration/settings overlays (`add/edit-connection`, `ai-gateway-consent`, `settings`, `api-keys`, `integrations`) **stay in `components/overlays/`** — opened by the `shared/ui` integration widgets, so moving them to a feature would create `shared→feature` edges; they move to platform in Phase 7 with the integration widgets. Quote-aware codemod + fixed cross-group relatives. tsc 0; **verified no `shared→feature` deps.**
