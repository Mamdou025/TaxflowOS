...
import type { SourceRow as ManualTableRow } from './rows';
...
export function parseHttpJsonConfig(config: Record<string, unknown>): HttpJsonSourceConfig {
  ...
  return {
    ...
    defaultAmount:
      config.defaultAmount !== null &&
      config.defaultAmount !== '' &&
      Number.isFinite(Number(config.defaultAmount))
    ...
    responseMeta: asRecord(config.responseMeta) as HttpJsonResponseMeta | undefined,
...
export function assertPublicHttpUrl(rawUrl: string): URL {
  ...
  if (url.protocol !== 'https:' && url.protocol !== 'http:') {
    throw new Error(`Only http and https URLs can be called — got "${url.protocol}".`);
  }
...
function readField(record: Record<string, unknown>, key: string | undefined): unknown {
  if (!key) {
    return;
  }
...
function guessKey(
  ...
  for (const hint of hints) {
    const hit = keys.find((key) => key.toLowerCase().includes(hint) && predicate(record[key]));
    if (hit) {
      return hit;
    }
  }
...
export function mapRecordsToRows({
  ...
  const labelKey =
    fieldMap.label || guessKey(first, LABEL_HINTS, (value) => typeof value === 'string');
  const amountKey =
    fieldMap.amount || guessKey(first, AMOUNT_HINTS, (value) => coerceNumber(value) !== null);
  ...
  slice.forEach((entry, index) => {
    ...
    const amount = coerceNumber(readField(record, amountKey)) ?? defaultAmount ?? null;
    if (amount === null) {
      skipped += 1;
      return;
    }

    const label = coerceText(readField(record, labelKey)) || `Record ${index + 1}`;

    rows.push({
      account: coerceText(readField(record, fieldMap.account)),
      amount,
      currency: coerceText(readField(record, fieldMap.currency)) || currency || 'USD',
      description: coerceText(readField(record, fieldMap.description)),
      label,
      raw: record,
      rowId: coerceText(readField(record, fieldMap.rowId)) || `api-row-${index + 1}`,
      rowNumber: index + 1,
    });
  });
...
export type FetchJsonPayloadOptions = {
  fetchImpl?: typeof fetch;
  maxAttempts?: number;
  retryDelayMs?: number;
};

export class HttpJsonSourceError extends Error {
  constructor(
    readonly code: 'REQUEST_FAILED' | 'HTTP_ERROR' | 'RESPONSE_TOO_LARGE' | 'INVALID_JSON',
    message: string,
    readonly retryable: boolean,
    readonly attempts: number,
  ) {
    super(message);
    this.name = 'HttpJsonSourceError';
  }
}
...
async function retryDelay(delayMs: number, attempt: number): Promise<void> {
  const duration = Math.min(2000, delayMs * 2 ** Math.max(0, attempt - 1));
...
export async function fetchJsonPayload(
  ...
  options: FetchJsonPayloadOptions = {},
  ...
  const retryableMethod = config.method === 'GET';
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      ...
      throw new HttpJsonSourceError(
        'REQUEST_FAILED',
        `Request to ${url.host} failed after ${attempt} attempts: ${reason}`,
        retryableMethod,
        attempt,
      );
    }
    ...
    if (!response.ok) {
      const retryable = retryableMethod && isRetryableStatus(response.status);
  ...
  throw new Error('Unreachable HTTP source retry state.');
}
