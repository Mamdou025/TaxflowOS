import { buildApiRequest } from './connectors';
import { extractAtPath, fetchJsonPayload, mapRecordsToRows } from './http-json';
...
export function parseCurrencyRateConfig(config: Record<string, unknown>): CurrencyRateConfig {
  const fapiInputs =
  ...
  return {
    documentCurrency:
      optionalString(source.documentCurrency) || optionalString(source.sourceCurrency) || 'USD',
    ...
    reportingCurrency:
      optionalString(source.reportingCurrency) || optionalString(source.targetCurrency) || 'CAD',
  };
}
