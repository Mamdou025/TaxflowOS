...
import type { HttpJsonFieldMap } from './http-json';
...
const usaspendingAwards: ApiConnector = {
  ...
  build: (params) => {
    const agency = text(params, 'agency', 'Department of Health and Human Services');
    ...
    const awardTypeCodes =
      awardType === 'grants' ? GRANTS : awardType === 'all' ? [...CONTRACTS, ...GRANTS] : CONTRACTS;
...
const nagerPublicHolidays: ApiConnector = {
  ...
  description: 'Every public holiday for one country and year, tagged national or regional.',
...
export function withParamDefaults(
  ...
  return Object.fromEntries(
    connector.params.map((param) => {
      ...
      const isBlank = supplied === undefined || supplied === null || supplied === '';
 ...
 */
export function validateApiParams(connector: ApiConnector, params: ApiParamValues = {}): string[] {
  ...
  return connector.params.flatMap((param) => {
    ...
    if (param.kind === 'currency' && !/^[A-Za-z]{3}$/.test(String(value))) {
      return [`"${param.label}" must be a three-letter currency code — got "${value}".`];
    }

    if (param.kind === 'enum' && param.options) {
      const allowed = param.options.map((option) => option.value);
      if (!allowed.includes(String(value))) {
        return [`"${param.label}" must be one of: ${allowed.join(', ')} — got "${value}".`];
      }
    }
...
export type BuildApiRequestResult =
  { ok: true; spec: ApiRequestSpec; params: ApiParamValues } | { ok: false; errors: string[] };
...
export function getParamOptionsSource(
  ...
  return getApiConnector(connectorId)?.params.find((param) => param.key === paramKey)
    ?.optionsSource;
}
...
export function optionsFromPayload(
  ...
  const pattern = source.valuePattern ? new RegExp(source.valuePattern) : undefined;
  ...
  const options = entries.flatMap(([rawValue, detail]) => {
    ...
    const label = source.labelKey ? String(detail?.[source.labelKey] ?? value) : value;
    return [{ label, value }];
  });
