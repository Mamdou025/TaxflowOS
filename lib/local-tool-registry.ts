...
type FinalityStatus = "draft" | "failed" | "final" | "review_ready";
...
const DEFAULT_TABLE_ROWS: FiscalRow[] = [
  {
    account: "4000",
    amount: 12_000,
...
function getConfiguredRows(config: Record<string, unknown>): FiscalRow[] {
  const rowSource =
    config.manualRows || config.tableRows || config.rows || config.sampleRows;
...
function getConfiguredScalar({
  ...
  const fromConfig =
    parseNumber(config.value) ??
    parseNumber(config.manualValue) ??
    parseNumber(config.scalarValue) ??
    parseNumber(config.valuePreview) ??
    parseNumber(block.source?.valuePreview);
...
function sourceTraceForEvidence(evidence: EvidenceRef): SourceTraceRef {
  return {
...
function sourceEvidenceForKeywordRule({
  block,
  rule,
}: {
  block: WorkflowBlock;
  ...
  return {
    ...
    locator:
      ...
      String(block.config.sourceLocator || "keyword-rules"),
    rowId: rule.ruleId,
    sourceBlockId: block.id,
    sourceLabel: block.label,
...
function getKeywordRuleMatchMode(value: unknown): KeywordRule["matchMode"] {
...
function fiscalRowFromOutputRecord({
  record,
  result,
}: {
...
function collectRowsFromResult(result: ToolRunResult): FiscalRow[] {
...
function collectRows(context: ToolExecutionContext): FiscalRow[] {
  return context.upstreamResults.flatMap(collectRowsFromResult);
}

function collectUnmatchedRows(context: ToolExecutionContext): FiscalRow[] {
...
function collectEvidence(context: ToolExecutionContext): EvidenceRef[] {
...
function collectSourceTrace(context: ToolExecutionContext): SourceTraceRef[] {
...
function keywordRuleFromConfigRecord(
  ...
  return {
    categoryId,
    categoryLabel: String(
      record.categoryLabel || record.label || categoryId.replaceAll("_", " ")
    ),
    ...
    ruleId: String(
      record.ruleId ||
        record.id ||
        `keyword-rule-${String(categoryId || index + 1)}`
    ),
    sectionId: getStringField(record, "sectionId"),
    scope: getStringField(record, "scope"),
    suggestedLine:
      getStringField(record, "suggestedLine") ||
      getStringField(record, "lineId"),
    suggestedSection:
      getStringField(record, "suggestedSection") ||
      getStringField(record, "sectionId"),
    suggestedSubsection:
      getStringField(record, "suggestedSubsection") ||
      getStringField(record, "subsectionId"),
...
function parseKeywordRules(value: unknown): KeywordRule[] {
  if (!Array.isArray(value)) {
...
function findKeywordRuleMatches(row: FiscalRow, rules: KeywordRule[]) {
  return rules.filter((rule) => Boolean(getRuleMatchedKeyword(row, rule)));
}

function getMatchedKeyword(row: FiscalRow, rule: KeywordRule) {
...
function createMappedKeywordRow({
  matchedKeyword,
  row,
  rule,
}: {
...
function createKeywordMapperWarnings({
  conflicts,
  lowConfidenceRows,
  rules,
  threshold,
  ...
  return [
    rules.length === 0
      ? "Keyword Mapper needs a connected Keyword Rulebook."
      : "",
...
function getKeywordRuleSourceLabels(context: ToolExecutionContext) {
...
type NumericValueRef = { key: string; label: string; value: number };
...
function collectScalarNumericValues({
  ...
  return [
    ...
    return value === null
      ? []
      : [{ key: `${result.blockId}.${key}`, label, value }];
  });
}
...
function getFxRateReviewDraft(context: ToolExecutionContext) {
  const candidate = getExchangeRateCandidate(context);
  const useOverride =
    context.config.useOverride === true ||
    context.config.useOverride === "true";
  const overrideRate = parseNumber(context.config.overrideRate);
  const overrideReason =
    typeof context.config.overrideReason === "string"
      ? context.config.overrideReason.trim()
      : "";
  const reviewedRate =
    useOverride && overrideRate !== null ? overrideRate : candidate?.rate;
...
function getFxRateReviewWarnings({
  approved,
  candidate,
  overrideRate,
  overrideReason,
...
function getFxRateReviewOutput({
  context,
  pass,
  warnings,
}: {
  ...
  const reviewer =
    context.config.reviewer || context.config.owner || "Reviewer";
  const exchangeRateInfo = {
  ...
  const validationResult = {
    message: pass
      ? "FX rate is reviewed and ready for protected use."
      : warnings[0],
...
function resolveOperand(
  ...
  const match = values.find(
    (item) =>
      item.key.toLowerCase() === ref ||
      item.key.toLowerCase().endsWith(`.${ref}`) ||
      item.label.toLowerCase() === ref ||
      item.label.toLowerCase().includes(ref)
  );

  return match?.value;
}

function getFormulaOperation(config: Record<string, unknown>) {
  const operation = String(config.operation || "").toLowerCase();
  if (
    ["add", "divide", "multiply", "percentage", "subtract"].includes(operation)
  ) {
    return operation as
      | "add"
      | "divide"
...
function calculateFormula(
  operation: ReturnType<typeof getFormulaOperation>,
  operands: number[]
) {
  ...
  if (operation === "subtract") {
    return operands
      .slice(1)
      .reduce((result, value) => result - value, operands[0]);
  }
...
function getFinalTotalForResult(
  context: ToolExecutionContext,
  resultName: string
) {
  ...
  for (const result of context.upstreamResults) {
    ...
    const value =
      parseNumber(finalTotal) ?? parseNumber(finalTotals?.[resultName]);
    if (value !== null) {
      return value;
    }
...
function getProtectedValue(context: ToolExecutionContext, resultName?: string) {
  const configured =
...
function formatSourceTraceRef(trace: SourceTraceRef) {
  const itemId = trace.rowId || trace.evidenceRefId?.split(":").at(-1);
  return itemId ? `${trace.sourceLabel}.${itemId}` : trace.sourceLabel;
}

function getPipelineTrace(context: ToolExecutionContext, resultName?: string) {
  if (
  ...
  if (
    resultName === "Z" ||
    context.workflow.name === "Single Item Pipeline Demo"
  ) {
    return [
...
function getCurrencyForNamedResult(result: ToolRunResult, resultName: string) {
  ...
  return typeof officialLineDetail?.currency === "string"
    ? officialLineDetail.currency
    : null;
}

function getUpstreamCurrency(
  context: ToolExecutionContext,
  resultName?: string
) {
  for (const result of context.upstreamResults) {
  ...
  return typeof context.config.currency === "string"
    ? context.config.currency
    : undefined;
}

function findProtectedResult(results: ToolRunResult[]) {
  for (const result of results) {
    const protectedResult =
      asRecord(result.output.protectedResult) ||
      asRecord(result.output.protected_result);
    if (protectedResult) {
...
function findValidationResult(results: ToolRunResult[]) {
  for (const result of results) {
    const validationResult =
...
function findApprovalStatus(results: ToolRunResult[]) {
  for (const result of results) {
    const approvalStatus =
      asRecord(result.output.approvalStatus) ||
      asRecord(result.output.approval_status);
    if (approvalStatus) {
...
function isUnmatchedRowsReview({
  ...
  return (
    result.toolId === "review.unmatched_rows_check" ||
    block?.subtype === "Unmatched Rows Check"
  );
}
...
function summarizeValidationGateResults({
  results,
  workflow,
}: {
  results: ToolRunResult[];
  ...
  const validationSummary = results.flatMap<ValidationGateIssue>((result) => {
    ...
    const blocking =
      result.output.blocking !== false && validationResult?.blocking !== false;
    const overridden =
      isUnmatchedRowsReview({ block, result }) &&
  ...
  const blockingIssues = validationSummary.filter(
    (item) => item.blocking && !item.pass
  );
...
function getProtectedResultsFinality(
  ...
  return protectedResults.map((result) => ({
    ...
    finalityStatus: String(
      result.finalityStatus ||
        (result.final === true ? "final" : "review_ready")
    ),
    name: result.name,
...
function getOutputFinalitySummary({
  protectedResults,
  results,
  workflow,
}: {
  ...
  const protectedResultsFinality =
    getProtectedResultsFinality(protectedResults);
...
function buildFinalityPreviewHeader(
  ...
  return [
    ...
    ...(finality.nonBlockingWarnings.length > 0
      ? finality.nonBlockingWarnings.map(
          (issue) => `- ${issue.label}: ${issue.message}`
        )
      : ["- None"]),
  ].join("\n");
...
function withCanonicalFinality(
  value: unknown,
  finality: ReturnType<typeof getOutputFinalitySummary>
) {
  return {
...
function hasRowsOutput(result: ToolRunResult) {
  ...
  return (
    ...
    (Array.isArray(result.output.selectedRows) &&
      result.output.selectedRows.length > 0) ||
...
function hasKeywordRulesOutput(result: ToolRunResult) {
  return (
    (Array.isArray(result.output.keywordRules) &&
      result.output.keywordRules.length > 0) ||
...
function hasAggregationRulesOutput(result: ToolRunResult) {
  return (
    (Array.isArray(result.output.aggregationRules) &&
      result.output.aggregationRules.length > 0) ||
    (parseNumber(result.output.ruleCount) ?? 0) > 0
  );
}
...
function hasRollupRulesOutput(result: ToolRunResult) {
  return (
    Array.isArray(result.output.rollupRules) &&
    result.output.rollupRules.length > 0
  );
}

function hasCalculationRulesOutput(result: ToolRunResult) {
  return (
    Array.isArray(result.output.calculationRules) &&
    result.output.calculationRules.length > 0
  );
}
...
function hasKnownRequiredInput({
  ...
  if (
    ["datarows", "rows", "selectedrows", "trialbalancerows"].includes(
      normalized
    )
  ) {
...
function hasRequiredInputKey(context: ToolExecutionContext, key: string) {
  const normalized = key.toLowerCase().replace(/[^a-z0-9]/g, "");

  return context.upstreamResults.some((result) => {
    const fapiInputs = getFapiInputsFromResult(result);
    const exchangeRateInfo =
      asRecord(result.output.exchangeRateInfo) ||
      getBackendOutputRole(result, "exchange_rate");
    ...
    return (
      parseNumber(fapiInputs?.[key]) !== null ||
      result.output[key] !== undefined
    );
  });
}

function getUnmatchedRowsReviewConfig(context: ToolExecutionContext) {
  ...
  const overrideReason =
    String(
      context.config.overrideReason || reviewOverride?.overrideReason || ""
    ).trim() || undefined;
...
function getUnmatchedRowsWarning({
  count,
  overrideUnmatchedRows,
}: {
  count: number;
...
function getUnmatchedRowsMessage({
  hasUnmatchedRows,
  pass,
  warning,
}: {
...
function getUnmatchedRowsStatus({
  pass,
  warnings,
}: {
  pass: boolean;
...
function getProtectedApprovalState(upstreamResults: ToolRunResult[]) {
  ...
  const approved =
    approvalPresent &&
    approvalResults.every((result) => result.output.approved === true);
  const approvedBy = approvalResults.find(
    (result) => result.output.approved === true
  )?.output.reviewer;

  return { approvalPresent, approved, approvedBy };
}

function getProtectedFinalityStatus({
  approvalPresent,
  approved,
  blockingIssues,
  value,
...
function getProtectedFinalityReason(finalityStatus: FinalityStatus) {
  if (finalityStatus === "final") {
...
function isDualResultMappingWorkflow(workflowName: string) {
  return (
...
function getDualResultTraceCommon(workflowName: string) {
  return workflowName === WORKING_SOURCE_RULES_DEMO_NAME
    ? WORKING_SOURCE_RULES_TRACE_COMMON
    : EXPANDED_MAPPING_TRACE_COMMON;
}

function getDualResultPipelineTrace(workflowName: string, resultName?: string) {
  ...
  const approvalStep =
    resultName === "W" ? "Approval Gate W" : "Approval Gate Z";
  const protectedStep =
    resultName === "W" ? "Protected Result W" : "Protected Result Z";
...
function getZOutputDetails({
  ...
  return {
    ...
    reviewer: firstPresent(
      approvalStatus?.reviewer,
      protectedResult.approvedBy
    ),
    ...
    status: firstPresent(
      ...
      final ? "locked" : "needs_review"
    ),
    subsectionId: firstPresent(
      mappedRow.subsectionId,
      ruleUsed?.subsectionId,
      "interest"
    ),
...
function buildZEvidencePreview({
  approvalStatus,
  mappedRows,
  protectedResult,
  ruleUsed,
...
function getProtectedResults(
  ...
  return results
    .map((result) => {
      ...
      return { ...protectedResult, blockId: result.blockId } as Record<
        string,
        unknown
      >;
    })
...
function findProtectedResultByName(
...
function getResultRows({
  ...
  if (workflowName === WORKING_SOURCE_RULES_DEMO_NAME) {
    const categoryIds =
      resultName === "Z" ? WORKING_Z_CATEGORY_IDS : WORKING_W_CATEGORY_IDS;
...
function getExpandedWarnings({
  ...
  return [
    ...lowConfidenceRows.map(
      (row) => `${row.rowId} confidence below threshold`
    ),
...
function findMappingSummary(results: ToolRunResult[]) {
  return results
    .map((result) => asRecord(result.output.mappingSummary))
    .find(Boolean);
}

function applyCombinedAggregationDetails(
  details: Record<string, unknown>,
  result: ToolRunResult
) {
...
function applySplitRollupDetails(
  details: Record<string, unknown>,
  result: ToolRunResult
) {
  ...
  details.categoryTotalDetails =
    result.output.categoryTotalDetails || details.categoryTotalDetails;
  details.categoryTotals =
    result.output.categoryTotals || details.categoryTotals;
...
function applySplitCalculationDetails(
  details: Record<string, unknown>,
  result: ToolRunResult
) {
...
function getAggregationRuleCount(results: ToolRunResult[]) {
  ...
  return Array.isArray(details?.aggregationTree)
    ? details.aggregationTree.length
    : 0;
}
...
function buildExpandedMappingSummary({
  ...
  const totalRows =
    parseNumber(summary?.totalRows) ?? mappedRows.length + unmatchedRows.length;

  return {
    lowConfidenceRows:
      parseNumber(summary?.lowConfidenceCount) ?? lowConfidenceRows.length,
...
function getSourceBlockSummary(context: ToolExecutionContext) {
  return context.workflow.blocks
    ...
    .map((block) => {
      ...
      const rules = Array.isArray(block.config.keywordRules)
        ? block.config.keywordRules
        : [];
...
function buildExpandedEvidencePreview({
  ...
  return [
    ...
    ...(sourceWarnings.length > 0
      ? sourceWarnings.map((warning) => `- ${warning}`)
      : ["- None"]),
    "",
    "Trace:",
    ...trace.map((step) => `- ${step}`),
...
function buildExpandedCanonicalJson({
  context,
  keywordRulesUsed,
  lowConfidenceRows,
  mappedRows,
...
function buildGenericEvidencePreview({
  mappedRows,
  protectedResults,
  results,
  sourceRows,
  ...
  return [
    ...
    "Rollup totals:",
    ...formatPreviewTotals(
      aggregationDetails?.rollupTotals || aggregationDetails?.groupTotals
    ),
    "Rollup / formula node totals:",
    ...formatPreviewTotals(aggregationDetails?.nodeTotals),
    "Formula totals:",
...
function buildGenericCanonicalJson({
  context,
  lowConfidenceRows,
  mappedRows,
  protectedResults,
...
function getToolInputSchema(fields: ToolSchemaField[]): ToolInputSchema {
  return { fields };
}

function getToolOutputSchema(fields: ToolSchemaField[]): ToolOutputSchema {
  return { fields };
}

const DATA_ROWS_INPUT_ROLE: ToolInputRole = {
...
const KEYWORD_RULES_INPUT_ROLE: ToolInputRole = {
...
const MAPPED_ROWS_INPUT_ROLE: ToolInputRole = {
...
const CHECKED_ITEMS_INPUT_ROLE: ToolInputRole = {
...
const REVIEW_FINDINGS_INPUT_ROLE: ToolInputRole = {
  acceptedFamilies: ["Logic", "Review / Validation"],
  acceptedOutputTypes: [
    "aggregation_summary",
    "calculation_summary",
...
const APPROVED_VALUE_INPUT_ROLE: ToolInputRole = {
  acceptedFamilies: ["Logic", "Review / Validation"],
  acceptedOutputTypes: [
    "calculated_results",
    "final_totals",
...
const VALUE_TO_APPROVE_INPUT_ROLE: ToolInputRole = {
...
const VALIDATION_RESULT_INPUT_ROLE: ToolInputRole = {
...
const CANDIDATE_VALUE_INPUT_ROLE: ToolInputRole = {
  acceptedFamilies: ["Logic"],
  acceptedOutputTypes: [
    "calculated_results",
    "final_totals",
...
const APPROVAL_STATUS_INPUT_ROLE: ToolInputRole = {
  acceptedFamilies: ["Review / Validation"],
...
const PROTECTED_VALUES_INPUT_ROLE: ToolInputRole = {
...
const PROTECTED_RESULT_INPUT_ROLE: ToolInputRole = {
...
const SOURCE_TRACE_INPUT_ROLE: ToolInputRole = {
...
const ROWS_OUTPUT_ROLE: ToolOutputRole = {
...
const KEYWORD_RULES_OUTPUT_ROLE: ToolOutputRole = {
  canRouteToFamilies: ["Logic"],
...
const VALUE_OUTPUT_ROLE: ToolOutputRole = {
...
const MAPPED_ROWS_OUTPUT_ROLE: ToolOutputRole = {
...
const UNMATCHED_ROWS_OUTPUT_ROLE: ToolOutputRole = {
...
const LOW_CONFIDENCE_ROWS_OUTPUT_ROLE: ToolOutputRole = {
  canRouteToFamilies: ["Review / Validation"],
...
const CONFLICTS_OUTPUT_ROLE: ToolOutputRole = {
  canRouteToFamilies: ["Review / Validation"],
...
const SUBTOTAL_OUTPUT_ROLE: ToolOutputRole = {
...
const INCLUDED_ROWS_OUTPUT_ROLE: ToolOutputRole = {
...
const EXCLUDED_ROWS_OUTPUT_ROLE: ToolOutputRole = {
  canRouteToFamilies: ["Output", "Review / Validation"],
...
const REVIEW_STATUS_OUTPUT_ROLE: ToolOutputRole = {
...
const VALIDATION_RESULT_OUTPUT_ROLE: ToolOutputRole = {
...
const APPROVAL_STATUS_OUTPUT_ROLE: ToolOutputRole = {
  canRouteToFamilies: ["Protected", "Output"],
...
const REVIEWED_EXCHANGE_RATE_OUTPUT_ROLE: ToolOutputRole = {
...
const GOVERNED_VALUE_OUTPUT_ROLE: ToolOutputRole = {
...
const PROTECTED_RESULT_OUTPUT_ROLE: ToolOutputRole = {
  canRouteToFamilies: ["Output"],
...
const OUTPUT_PACKAGE_ROLE: ToolOutputRole = {
  canRouteToFamilies: [],
...
function createMockParserTool({
  ...
  return {
    ...
    execute: (context) => {
      ...
      const warning =
        "Mock parser only in v1. Real parser integration comes later.";
    ...
    inputSchema: getToolInputSchema([
      { key: "sourceEvidence", type: "object" },
    ]),
    outputRoles: [
      {
        ...ROWS_OUTPUT_ROLE,
        description:
          "Mock parser output rows that preserve lineage to the Source.",
...
function normalizeResultLookupKey(resultName: string) {
  return resultName
    .trim()
    .toUpperCase()
    .replace(/[^A-Z0-9]+/g, "_")
...
function getNumberRecordValueByResultName(
...
function collectExpectedResultValues(context: ToolExecutionContext) {
  ...
  for (const result of context.upstreamResults) {
    ...
    collectNumberRecord(
      expectedResults,
      asRecord(result.output.fapiInputs)?.expectedResults
    );
  }
...
function collectActualResultValues(context: ToolExecutionContext) {
  ...
  for (const result of context.upstreamResults) {
    ...
    const protectedName = String(
      protectedResult?.resultName || protectedResult?.name || ""
    );
...
const localTools: ToolDefinition[] = [
  {
    defaultConfig: { rows: DEFAULT_TABLE_ROWS },
    ...
    execute: (context) => {
      ...
      const evidenceRefs = rows.map((row) =>
        sourceEvidenceForRow({ block: context.block, row })
      );
        ...
        output: {
          ...
          rows: rows.map((row) => ({
            ...
            evidenceRefs: evidenceRefs.filter(
              (evidence) => evidence.rowId === row.rowId
            ),
            sourceTrace: sourceTrace.filter(
              (trace) => trace.rowId === row.rowId
            ),
          })),
    ...
    execute: (context) => {
      ...
      const evidence: EvidenceRef = {
        ...
        locator:
          ...
          String(context.config.sourceLocator || "manual-value"),
      ...
      return completeResult({
        ...
        sourceTrace: [sourceTraceForEvidence(evidence)],
        status: "success",
      });
    },
    family: "Source",
  ...
  },
  {
    ...
    execute: (context) => {
      ...
      const evidenceRefs = rules.map((rule) =>
        sourceEvidenceForKeywordRule({ block: context.block, rule })
      );
      const sourceTrace = evidenceRefs.map(sourceTraceForEvidence);
    ...
    inputRoles: [],
    inputSchema: getToolInputSchema([]),
    outputRoles: [KEYWORD_RULES_OUTPUT_ROLE],
    outputSchema: getToolOutputSchema([
      { key: "keywordRules", required: true, type: "array" },
      { key: "immutable", required: true, type: "boolean" },
    ]),
    runMode: "local_mock",
  ...
  {
    defaultConfig: { lowConfidenceThreshold: 0.75 },
    ...
    execute: (context) => {
      ...
      const lowConfidenceThreshold =
        parseNumber(context.config.lowConfidenceThreshold) ?? 0.75;
      const mappedRows: FiscalRow[] = [];
      const unmatchedRows: FiscalRow[] = [];
      ...
      const lowConfidenceRows = mappedRows.filter(
        (row) => (row.confidence ?? 1) < lowConfidenceThreshold
      );
      const confidence =
        average(mappedRows.map((row) => row.confidence || 0).filter(Boolean)) ??
        0;
      const categoryCounts = mappedRows.reduce<Record<string, number>>(
        (counts, row) => {
      ...
      const categoryAmountTotals = mappedRows.reduce<Record<string, number>>(
        (totals, row) => {
          const categoryId = row.categoryId || "uncategorized";
    ...
    family: "Logic",
    inputRoles: [DATA_ROWS_INPUT_ROLE, KEYWORD_RULES_INPUT_ROLE],
    inputSchema: getToolInputSchema([
      { key: "rows", required: true, type: "array" },
      { key: "keywordRules", required: true, type: "array" },
    ]),
    ...
    outputSchema: getToolOutputSchema([
      { key: "mappedRows", required: true, type: "array" },
  ...
  },
  {
    ...
    execute: (context) => {
      ...
      const includeSubsectionIds = asStringArray(
        context.config.includeSubsectionIds
      );
      ...
      const includedRows = rows.filter((row) => {
        ...
        const included =
          ...
          (includeSectionIds.length === 0 ||
            includeSectionIds.includes(sectionId)) &&
          (includeSubsectionIds.length === 0 ||
            includeSubsectionIds.includes(subsectionId));
        const excluded =
          excludeTargets.length > 0 && excludeTargets.includes(target);
      ...
      const currency =
        includedRows.find((row) => row.currency)?.currency ||
        String(context.config.currency || "");
  ...
  {
    defaultConfig: { operation: "multiply" },
    description: "Calculates a value using safe local operations only.",
    displayName: "Formula",
    execute: (context) => {
      const numericValues = collectNumericValues(context);
      const configuredOperands = Array.isArray(context.config.operands)
        ? context.config.operands
        : [];
      const operands =
        configuredOperands.length > 0
          ? configuredOperands
              .map((operand) => resolveOperand(operand, numericValues))
              .filter(
                (resolvedValue): resolvedValue is number =>
                  typeof resolvedValue === "number"
              )
          : numericValues.map((item) => item.value);
      const operation = getFormulaOperation(context.config);
      const formulaValue = calculateFormula(operation, operands);
      const warnings =
        formulaValue === null ? ["No numeric operands were available."] : [];
      ...
      return completeResult({
        ...
        logs: [
          makeLog({
            blockId: context.block.id,
            details: { operands, operation, value: formulaValue },
            level: warnings.length > 0 ? "warning" : "info",
    ...
    inputSchema: getToolInputSchema([{ key: "values", type: "array" }]),
    outputRoles: [
      {
        ...VALUE_OUTPUT_ROLE,
        canRouteToFamilies: ["Protected", "Review / Validation"],
    ...
    outputSchema: getToolOutputSchema([{ key: "value", type: "number" }]),
    runMode: "local_mock",
    subtype: "Formula",
    toolGroup: "calculation",
  ...
  },
  {
    ...
    description:
      "Creates downstream transformed copies while preserving lineage.",
  ...
  }),
  {
    ...
    execute: (context) => {
      ...
      const missingRequiredKeys = requiredKeys.filter(
        (key) => !hasRequiredInputKey(context, key)
      );
      const missingUpstream =
        context.upstreamResults.length === 0
      ...
      const validationResult = {
        ...
        message: pass
          ? "Required FAPI workflow inputs are present."
          : warnings[0],
      ...
      return completeResult({
        ...
        output: {
          ...
          validationResult: {
            ...
            message:
              warnings[0] ||
              "All mapped rows meet the low-confidence warning threshold.",
            pass: warnings.length === 0,
  ...
  },
  {
    ...
    description:
      "Checks whether mapped rows meet a configured confidence threshold.",
  ...
  },
  {
    ...
    description:
      "Compares actual calculator results against expected workbook results.",
    ...
    execute: (context) => {
      ...
      const checks = Object.entries(expectedResults).map(
        ([resultName, expected]) => {
          const actual = getNumberRecordValueByResultName(
            actualResults,
            resultName
          );
          const delta =
            typeof actual === "number"
              ? Number((actual - expected).toFixed(6))
              : null;
          return {
            actual,
            delta,
      ...
      const validationResult = {
        ...
        message: pass
          ? "Formula results match the expected workbook values."
          : warnings[0],
        pass,
        status: pass ? "pass" : "warning",
        tolerance,
  ...
  },
  {
    ...
    inputSchema: getToolInputSchema([
      { key: "exchangeRate", required: true, type: "object" },
    ]),
    outputRoles: [
  ...
  },
  {
      ...
      return completeResult({
        ...
        logs: [
          makeLog({
            ...
            message: approved
              ? "Approval gate approved locally."
              : "Approval gate needs review.",
          }),
        ],
        output: {
          approved,
    ...
    inputSchema: getToolInputSchema([
      { key: "reviewFindings", type: "object" },
    ]),
    outputRoles: [
      APPROVAL_STATUS_OUTPUT_ROLE,
      {
  ...
  },
  {
    ...
    execute: (context) => {
      ...
      const requiredProtectedResults = asStringArray(
        context.config.requiredProtectedResults
      );
      ...
      const missingItems = [
        ...
        ...missingProtectedResults.map(
          (name) => `Missing required protected result: ${name}.`
        ),
  ...
  },
  {
    ...
    execute: (context) => {
      ...
      const warnings =
        value === null ? ["No protected input value found."] : [];
      const fapiInputKey =
        typeof context.config.fapiInputKey === "string"
          ? context.config.fapiInputKey
          : undefined;
      const fapiInputs =
        fapiInputKey && value !== null ? { [fapiInputKey]: value } : undefined;
  ...
  },
  {
    ...
    execute: (context) => {
      ...
      const { approvalPresent, approved, approvedBy } =
        getProtectedApprovalState(context.upstreamResults);
    ...
    inputSchema: getToolInputSchema([
      { key: "approvedLogicResult", type: "object" },
    ]),
  ...
  },
  {
    ...
    execute: (context) => {
      ...
      const mappedFiscalRows = collectRowsByOutputKey(
        sourceResults,
        "mappedRows"
      );
      const lowConfidenceRows = collectRowsByOutputKey(
        sourceResults,
        "lowConfidenceRows"
      );
      const unmatchedRows = collectRowsByOutputKey(
        sourceResults,
        "unmatchedRows"
      );
      ...
      const expandedCanonicalJson = isDualResultMappingWorkflow(
      ...
      const protectedNeedsReview = protectedValues.some(
        (value) => value.runtimeLocked !== true
      );
      ...
      return completeResult({
        ...
        sourceTrace: collectSourceTrace(context),
        status:
          protectedNeedsReview || warnings.length > 0 ? "warning" : "success",
        warnings,
      });
    },
    family: "Output",
    ...
    inputSchema: getToolInputSchema([
      { key: "protectedValues", type: "array" },
    ]),
    outputRoles: [
      {
        ...OUTPUT_PACKAGE_ROLE,
  ...
  },
  {
    ...
    execute: (context) => {
      ...
      return completeResult({
        ...
        sourceTrace: collectSourceTrace(context),
        status:
          protectedNeedsReview || outputWarnings.length > 0
            ? "warning"
            : "success",
        warnings: outputWarnings,
      });
...
function pushFlattenedResultAsInputs(
  ...
  if (
    typeof result.output.namedValues === "object" &&
    result.output.namedValues
  ) {
  ...
  if (
    typeof result.output.calculatedResults === "object" &&
    result.output.calculatedResults
  ) {
    pushInputRole(inputsByRole, "named_values", {
  ...
  if (
    typeof result.output.fapiInputs === "object" &&
    result.output.fapiInputs
  ) {
...
function toBackendExecutionContext(
  context: ToolExecutionContext
...
function flattenBackendResultOutput(result: BackendToolRunResult) {
  ...
  const sourceMetadataOutput = getBackendOutputRecord(
    result,
    "source_metadata"
  );
  ...
  const aggregationRulesOutput = getBackendOutputRecord(
    result,
    "aggregation_rules"
  );
  if (Object.keys(aggregationRulesOutput).length > 0) {
    output.aggregationRuleEvidence =
      aggregationRulesOutput.aggregationRuleEvidence;
  ...
  const calculationRulesOutput = getBackendOutputRecord(
    result,
    "calculation_rules"
  );
  if (Object.keys(calculationRulesOutput).length > 0) {
    output.calculationRuleEvidence =
      calculationRulesOutput.calculationRuleEvidence;
  ...
  const lowConfidenceRowsOutput = getBackendOutputRecord(
    result,
    "low_confidence_rows"
  );
  ...
  const mappingSummaryOutput = getBackendOutputRecord(
    result,
    "mapping_summary"
  );
  ...
  const categoryTotalsOutput = getBackendOutputRecord(
    result,
    "category_totals"
  );
  ...
  const officialLineValuesOutput = getBackendOutputRecord(
    result,
    "official_line_values"
  );
  ...
  if (Object.keys(fapiInputsOutput).length > 0) {
    output.expectedResults = asRecord(
      fapiInputsOutput.fapiInputs
    )?.expectedResults;
  ...
  const aggregationTreeOutput = getBackendOutputRecord(
    result,
    "aggregation_tree"
  );
  ...
  const includedRowsByNodeOutput = getBackendOutputRecord(
    result,
  ...
  const hierarchyExcludedRowsOutput = getBackendOutputRecord(
    result,
    "excluded_rows"
  );
  ...
  const includedRowsByCategoryOutput = getBackendOutputRecord(
    result,
    "included_rows_by_category"
  );
  if (Object.keys(includedRowsByCategoryOutput).length > 0) {
    output.includedRowsByCategory =
      includedRowsByCategoryOutput.includedRowsByCategory;
  }

  const includedRowsByRollupOutput = getBackendOutputRecord(
    result,
    "included_rows_by_rollup"
  );
  if (Object.keys(includedRowsByRollupOutput).length > 0) {
    output.includedRowsByRollup =
      includedRowsByRollupOutput.includedRowsByRollup;
  }
  ...
  const rollupFormulaTraceOutput = getBackendOutputRecord(
    result,
  ...
  const aggregationSummaryOutput = getBackendOutputRecord(
    result,
    "aggregation_summary"
  );
  ...
  const calculatedResultsOutput = getBackendOutputRecord(
    result,
    "calculated_results"
  );
  ...
  const calculationSummaryOutput = getBackendOutputRecord(
    result,
    "calculation_summary"
  );
...
function createBackendAdaptedTool(toolId: string): ToolDefinition | null {
...
const BACKEND_ADAPTED_TOOL_ENTRIES: [string, ToolDefinition][] =
  BACKEND_ADAPTED_TOOL_IDS.flatMap((toolId) => {
    const tool = createBackendAdaptedTool(toolId);
    return tool ? [[toolId, tool]] : [];
  });

const BACKEND_ADAPTED_TOOLS: Record<string, ToolDefinition> =
  Object.fromEntries(BACKEND_ADAPTED_TOOL_ENTRIES);

export const LOCAL_TOOL_REGISTRY: Record<string, ToolDefinition> = {
  ...LOCAL_TOOLS_BY_ID,
  ...BACKEND_ADAPTED_TOOLS,
  "logic.keyword_classifier": BACKEND_ADAPTED_TOOLS[
    "logic.keyword_mapper"
  ] as ToolDefinition,
};
...
const TABLE_SOURCE_SUBTYPES = new Set<BlockSubtype>([
  "Database Query",
  "Excel / Workbook",
]);
...
function getSourceToolId(block: WorkflowBlock) {
  if (
    block.config.sourceKind === "fapi_inputs" ||
    block.config.toolId === "source.fapi_inputs"
  ) {
    return "source.fapi_inputs";
  }

  if (
    block.config.sourceKind === "currency_rate" ||
  ...
  if (
    block.config.sourceKind === "rollup_rules" ||
    block.catalogId === "source:rollup-rules"
  ) {
  ...
  if (
    ...
    block.catalogId === "source:keyword-rules"
  ) {
...
> = {
  ...
  Output: (block) =>
    OUTPUT_TOOL_BY_SUBTYPE[block.subtype] || "output.evidence_pack_preview",
  Protected: (block) =>
