...
import {
  ...
  type ToolOutputRole,
  type ToolRunLog,
} from '@workspace/workflow-executors/tools/types';

// ── local copies of the registry's tiny helpers ──────────────────────────────
// These builders stay private to the missing-tool definitions so the registry's
// public surface remains limited to lookup and resolution.
...
const schema = (fields: ToolSchemaField[]) => ({ fields });
...
function makePinnedSourceTool(input: {
  toolId: string;
  displayName: string;
  subtype: NonNullable<ToolDefinition['subtype']>;
  description: string;
  sourceKind: string;
  locatorKey: string;
  ...
  return {
    ...
    execute: (context) => {
      ...
      const { evidenceRefs, sourceTrace } = rowEvidence(context, rows, input.sourceKind, locator);
...
export const scheduleTriggerTool = makeTriggerTool({
  ...
  mode: 'schedule',
  description: 'Starts the workflow on a schedule and records the cadence the run belongs to.',
  detail: (config) => ({
...
const PROPOSAL_OUT: ToolOutputRole = {
  ...
  description: 'A model suggestion awaiting human acceptance. Never a governed value.',
  id: 'proposal',
  label: 'Proposal',
  outputKey: 'proposal',
  outputType: 'proposal',
