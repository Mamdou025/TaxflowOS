...
export function normalizeCalculationRule(value: unknown, index: number): CalculationRule | null {
  ...
  const resultKey =
    optionalString(record.resultKey) || optionalString(record.resultName) || calculationId;
...
export function parseCalculationRules({
  ...
  const ruleSource =
    config.calculationRules || config.calculation_rules || config.rules || config.nodes;
