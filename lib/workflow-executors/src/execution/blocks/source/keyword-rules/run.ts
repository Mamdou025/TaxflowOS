...
export function runKeywordRulesSource(context: ToolExecutionContext): ToolRunResult {
  ...
  const keywordRuleEvidence = Object.fromEntries(
    rules.map((rule) => [rule.ruleId, evidenceRefs.filter((ref) => ref.ruleId === rule.ruleId)]),
  );
  const keywordRuleTrace = Object.fromEntries(
    rules.map((rule) => [rule.ruleId, sourceTrace.filter((trace) => trace.ruleId === rule.ruleId)]),
  );
