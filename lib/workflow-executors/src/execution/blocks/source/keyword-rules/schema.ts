...
export type KeywordRuleMatchMode = 'contains' | 'exact' | 'starts_with' | 'all_words';
...
export function normalizeKeywordRule(value: unknown, index: number): KeywordRule | null {
  ...
  return {
    ...
    suggestedSubsection:
      optionalString(record.suggestedSubsection) || optionalString(record.subsectionId),
...
export function parseKeywordRules({
  ...
  if (!Array.isArray(ruleSource)) {
    return fallbackRules.map((rule) => ({
      ...rule,
      containsKeywords: rule.containsKeywords ? [...rule.containsKeywords] : undefined,
      exactKeywords: rule.exactKeywords ? [...rule.exactKeywords] : undefined,
      excludeKeywords: rule.excludeKeywords ? [...rule.excludeKeywords] : undefined,
