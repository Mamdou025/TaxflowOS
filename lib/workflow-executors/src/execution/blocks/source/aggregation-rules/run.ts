...
export function runAggregationRulesSource(context: ToolExecutionContext): ToolRunResult {
  ...
  const aggregationRuleEvidence = Object.fromEntries(
    rules.map((rule) => [rule.nodeId, evidenceRefs.filter((ref) => ref.ruleId === rule.nodeId)]),
  );
  const aggregationRuleTrace = Object.fromEntries(
    rules.map((rule) => [rule.nodeId, sourceTrace.filter((trace) => trace.ruleId === rule.nodeId)]),
  );
