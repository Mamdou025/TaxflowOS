...
export type AggregationNodeType =
  'category_total' | 'constant' | 'final_result' | 'formula' | 'group';
...
function normalizeRecordOperand(record: Record<string, unknown>): AggregationOperand | null {
  const refType =
    record.refType === 'category' || record.refType === 'constant' || record.refType === 'input'
      ? record.refType
      : 'node';
...
export function normalizeAggregationRule(value: unknown, index: number): AggregationRule | null {
  const record = asRecord(value);
  if (!record) {
    return null;
  }
  ...
  return {
    ...
    formulaExpression:
      optionalString(record.formulaExpression) || optionalString(record.expression),
...
export function parseAggregationRules({
  ...
  const ruleSource =
    config.aggregationRules || config.aggregation_rules || config.rules || config.nodes;
