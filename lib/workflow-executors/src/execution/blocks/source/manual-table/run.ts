...
export function runManualTableSource(context: ToolExecutionContext): ToolRunResult {
  ...
  const workbookFile =
    typeof context.config.workbookFile === 'object' && context.config.workbookFile !== null
  ...
  return {
    ...
    warnings: missingDocument
      ? []
      : rows.length === 0
        ? [missingMessage]
        : rows.some((row) => !Number.isFinite(row.amount))
          ? [
              'Some records have no numerical value. They can be classified; choose a number field before calculating amounts.',
            ]
          : [],
  };
}
