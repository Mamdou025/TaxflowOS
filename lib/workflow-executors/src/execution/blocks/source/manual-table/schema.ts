...
export function normalizeManualTableRow(value: unknown, index: number): ManualTableRow | null {
  ...
  const amount = Object.hasOwn(record, 'amount')
    ...
    : (parseNumber(record.value) ?? parseNumber(record.balance));
  ...
  return {
    ...
    raw: asRecord(record.raw) || asRecord(asRecord(record.metadata)?.raw) || record,
