...
function normalizeRtf(value: number | undefined): number {
  ...
  return (
    ALLOWED_RTF_VALUES.find((allowed) => Math.abs(allowed - value) < 1e-7) ?? DEFAULT_RTF_VALUE
  );
}
...
export function parseFapiInputsConfig(config: Record<string, unknown>): FapiInputsConfig {
  const fapiInputs =
  ...
  return {
    documentCurrency:
      optionalString(source.documentCurrency) || optionalString(source.sourceCurrency),
    ...
    reportingCurrency:
      optionalString(source.reportingCurrency) || optionalString(source.targetCurrency),
