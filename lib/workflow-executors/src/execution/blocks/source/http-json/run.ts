...
export function runHttpJsonSource(context: ToolExecutionContext): ToolRunResult {
  ...
  const origin = pinned.length > 0 ? 'live' : injected.length > 0 ? 'injected' : 'sample';
  if (
    (origin === 'sample' && context.config.useSampleData !== true) ||
    (origin === 'live' &&
      context.config.fetchedRequestSignature &&
      context.config.fetchedRequestSignature !== apiRequestSignature(context.config))
  ) {
    return {
      ...
      errors: [
        origin === 'live'
          ? 'API request settings changed. Fetch again before using the saved response.'
          : 'No API response is saved. Fetch the source or explicitly enable example data.',
      ],
      warnings: [],
      outputs: {},
      logs: [],
  ...
  const sourceTrace = evidenceRefs.map((evidenceRef) => createSourceTraceRef({ evidenceRef }));
