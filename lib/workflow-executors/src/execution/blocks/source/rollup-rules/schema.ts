...
export type RollupOperation =
  'sum' | 'sum_abs' | 'subtract' | 'multiply' | 'divide' | 'pass_through';
...
function parseOperation(value: unknown): RollupOperation {
  return ['sum', 'sum_abs', 'subtract', 'multiply', 'divide', 'pass_through'].includes(
    String(value),
  )
    ? (value as RollupOperation)
    : 'sum';
}
...
export function normalizeRollupRule(value: unknown, index: number): RollupRule | null {
  ...
  const rollupId =
    optionalString(record.rollupId) || optionalString(record.nodeId) || optionalString(record.id);
...
export function parseRollupRules({
  ...
  const ruleSource =
    config.rollupRules || config.rollup_rules || config.aggregationRules || config.rules;
