...
function rowFromRecord(record: Record<string, unknown>, index: number): HierarchyMappedRow | null {
  const amount = Object.hasOwn(record, 'amount')
    ? parseNumber(record.amount)
    : (parseNumber(record.value) ?? parseNumber(record.subtotal));
  const categoryId =
...
export function collectMappedRowsFromBackendInput(value: unknown): HierarchyMappedRow[] {
...
export function collectAggregationRulesFromBackendInput(value: unknown): AggregationRule[] {
  if (Array.isArray(value)) {
    return value
      .map((item) => asRecord(item))
      .filter((item): item is Record<string, unknown> => Boolean(item))
  ...
  for (const key of ['aggregationRules', 'aggregation_rules', 'rules', 'nodes']) {
