...
function buildImplicitOperands(rule: AggregationRule): AggregationOperand[] {
  ...
  const categoryIds =
    rule.nodeType === 'category_total' && (rule.includeCategoryIds || []).length === 0
      ? [rule.nodeId]
...
function createEmptyNodeTotal(rule: AggregationRule, message: string): NodeTotal {
...
function sortAggregationRules(rules: AggregationRule[]) {
  return rules
    ...
    .sort((left, right) => (left.rule.order ?? left.index) - (right.rule.order ?? right.index))
    .map(({ rule }) => rule);
}
...
function resolveConstantOperand(operand: AggregationOperand, sign: 1 | -1): ResolvedOperand {
...
export function runHierarchyAggregator(context: ToolExecutionContext): ToolRunResult {
  ...
  const categoryTotals = Object.fromEntries(
    Object.entries(categoryTotalDetails).map(([categoryId, detail]) => [categoryId, detail.value]),
  );
  const rulesById = new Map(aggregationRules.map((rule) => [rule.nodeId, rule]));
  ...
  function computeRuleTotal(rule: AggregationRule, visiting: Set<string> = new Set()): NodeTotal {
    ...
    const resolveFormulaReference = (reference: string): ResolvedOperand => {
      ...
      if (normalizedPrefix === 'constant' || normalizedPrefix === 'value') {
        return resolveConstantOperand({ refType: 'constant', value: Number(refId) || 0 }, 1);
      }
    ...
    const nodeTotal: NodeTotal = {
      ...
      currency:
        getCurrency(mappedRows.filter((row) => includedRows.includes(row.rowId))) ||
  ...
  const officialLineDetails = Object.fromEntries(
    aggregationRules
      ...
      .map((rule) => [rule.resultName || rule.nodeId, nodeTotalDetails.get(rule.nodeId)])
      .filter((entry): entry is [string, NodeTotal] => Boolean(entry[1])),
  );
  const officialLineValues = Object.fromEntries(
    Object.entries(officialLineDetails).map(([resultName, detail]) => [resultName, detail.value]),
  );
  ...
  const finalTotals = Object.fromEntries(
    Object.entries(finalTotalDetails).map(([resultName, detail]) => [resultName, detail.value]),
  );
  ...
  const formulaTrace = Object.fromEntries(
    [...nodeTotalDetails.values()].map((total) => [total.nodeId, total.formulaTrace]),
  );
  ...
  return {
    ...
    logs: [
      warnings.length > 0
        ...
        : info('Rollup & Calculation Engine completed through aggregation rules.', {
            ...aggregationSummary,
          }),
    ],
    outputs: {
