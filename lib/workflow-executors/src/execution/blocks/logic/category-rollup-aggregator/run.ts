...
import { evaluateRollupGroups } from '@workspace/workflow-executors/rollup-evaluation';
...
import { collectMappedRowsFromRollupInput, collectRollupRulesFromBackendInput } from './schema';
...
function getCategoryTotals(details: Record<string, CategoryDetail>) {
  return Object.fromEntries(
    Object.entries(details).map(([categoryId, detail]) => [categoryId, detail.value]),
  );
}

export function runCategoryRollupAggregator(context: ToolExecutionContext): ToolRunResult {
  ...
  const invalidRows = getRoleInputs(context, 'mapped_rows').flatMap((input) => {
    ...
    const raw = Array.isArray(input)
      ? input
      : (record?.mappedRows ?? record?.mapped_rows ?? record?.rows);
    return Array.isArray(raw)
      ? raw.filter((row) => collectMappedRowsFromRollupInput([row]).length === 0)
      : [];
  });
  if (invalidRows.length)
    return createErrorResult({
      ...
      errors: invalidRows.map(
        (row, index) =>
          `Cannot aggregate ${String(row?.label ?? row?.rowId ?? `row ${index + 1}`)}: its category or numeric value is missing. Review the document fields.`,
      ),
    });
  ...
  for (const input of getRoleInputs(context, 'mapping_summary')) {
    const declared = (input as { rulesUsed?: { categoryId?: string; categoryLabel?: string }[] })
      .rulesUsed;
    if (!Array.isArray(declared)) continue;
    for (const category of declared) {
      if (category.categoryId && !Object.hasOwn(categoryDetails, category.categoryId))
        categoryDetails[category.categoryId] = {
          categoryId: category.categoryId,
          categoryLabel: category.categoryLabel || category.categoryId,
          includedRows: [],
          rowCount: 0,
          value: 0,
        };
  ...
  try {
    ...
    return createErrorResult({
      context,
      errors: [error instanceof Error ? error.message : 'Could not evaluate aggregation groups.'],
    });
  }
  ...
  for (const rule of rules) {
    ...
    const rollupRows = rows.filter((row) => rollup.leafCategoryIds.includes(row.categoryId));
  ...
  const evidenceRefs = dedupeEvidenceRefs([...evidenceRows(rows), ...evidenceRules(rules)]);
  const rollupSummary = {
