...
function getRows(context: ToolExecutionContext): ManualTableRow[] {
  return getRoleInputs(context, 'data_rows').flatMap((input) => collectRowsFromBackendInput(input));
}
...
function getFieldValues(row: ManualTableRow, fields: KeywordMapperConfig['matchFields']) {
  return fields
    .map((field) => row[field])
...
function wordEquivalent(token: string, word: string) {
  ...
  const [shortWord, longWord] = token.length <= word.length ? [token, word] : [word, token];
...
function findMatches({
  ...
  return rules.flatMap((rule) => {
    ...
    const matchedKeyword = getKeywordCandidates(rule, config.matchMode).find((candidate) =>
      keywordMatches({
        fieldValues,
    ...
    return matchedKeyword ? [{ matchedKeyword: matchedKeyword.keyword, rule }] : [];
  });
}

function chooseMatch(matches: RuleMatch[], strategy: KeywordMapperConfig['conflictStrategy']) {
...
function getMissingInputErrors({ rows, rules }: { rows: ManualTableRow[]; rules: KeywordRule[] }) {
  return [
