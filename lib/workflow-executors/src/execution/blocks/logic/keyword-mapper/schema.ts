...
export type KeywordMapperConflictStrategy =
  'highest_confidence' | 'highest_priority' | 'send_conflict_to_review';

export type KeywordMapperUnmatchedStrategy = 'ignore' | 'mark_as_unmapped' | 'send_to_review';
...
function parseMatchFields(value: unknown): KeywordMapperMatchField[] {
  const allowed = new Set<KeywordMapperMatchField>(['account', 'description', 'label']);
  const fields = asStringArray(value).filter((field): field is KeywordMapperMatchField =>
...
function parseUnmatchedStrategy(value: unknown): KeywordMapperUnmatchedStrategy {
  if (value === 'ignore') {
    return 'ignore';
  }
...
export function parseKeywordMapperConfig(config: Record<string, unknown>): KeywordMapperConfig {
  return {
...
function manualRowFromRecord(rowRecord: Record<string, unknown>, index: number): ManualTableRow {
  return {
    ...rowRecord,
...
function keywordRuleFromRecord(ruleRecord: Record<string, unknown>, index: number): KeywordRule {
  ...
  return {
    categoryId,
    categoryLabel: String(
      ruleRecord.categoryLabel || ruleRecord.label || humanizeCategoryId(categoryId),
    ),
...
export function collectRulesFromBackendInput(value: unknown): KeywordRule[] {
  if (Array.isArray(value)) {
    return value
      ...
      .filter((rule) => rule.keywords.length > 0 && rule.categoryId !== 'mapped');
  }
