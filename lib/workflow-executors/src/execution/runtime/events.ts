...
export function info(message: string, details?: Record<string, unknown>): ToolRunLog {
  return makeToolLog({ details, level: 'info', message });
}

export function warning(message: string, details?: Record<string, unknown>): ToolRunLog {
  return makeToolLog({ details, level: 'warning', message });
}

export function error(message: string, details?: Record<string, unknown>): ToolRunLog {
