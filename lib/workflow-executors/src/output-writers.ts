 ...
 */
export function buildPdf({ pages, title }: { pages: string[][]; title: string }): string {
...
export function paginateTable({
  ...
  const widths = columns.map((column) =>
    Math.max(column.length, ...rows.map((row) => String(row[column] ?? '').length), 6),
  );
