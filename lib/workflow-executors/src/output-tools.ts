...
import { buildHandoff, buildPdf, paginateTable, type HandoffRow } from './output-writers';
import {
  type ToolDefinition,
...
export const pdfReportTool: ToolDefinition = {
  defaultConfig: {},
  description: "Renders the workflow's figures and rows as a real, downloadable PDF document.",
  ...
  execute: (context) => {
    ...
    const title = String(context.config.reportTitle || context.workflow.name || 'Workflow report');
    ...
    const pdfBase64 =
      typeof btoa === 'function' ? btoa(pdf) : Buffer.from(pdf, 'latin1').toString('base64');
