  ...
  const evaluate = (id: string): RollupEvaluation => {
    ...
    const inputValues = rule.includeCategoryIds.map((categoryId) => {
      ...
      if (!Object.hasOwn(categories, categoryId))
        throw new Error(
          `Unknown category reference: ${categoryId}. Connect the category's rules and data before calculating this group.`,
        );
      ...
      switch (rule.operation) {
        ...
        case 'divide':
          if (values.slice(1).includes(0))
            throw new Error(`Divide by zero in aggregation group ${id}.`);
          result = values.slice(1).reduce((a, b) => a / b, values[0]);
          break;
        case 'pass_through':
          result = values[0];
