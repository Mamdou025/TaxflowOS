...
export type ProtectedKind =
  'input' | 'result' | 'official-line' | 'locked-rate' | 'final-reviewed-amount' | 'summary-result';

export function getProtectedKindForSubtype(subtype: BlockSubtype): ProtectedKind {
 ...
 */
export function isGovernedValueBlock(block: GovernedValueProbe | null | undefined): boolean {
  if (!block) {
    return false;
  }
