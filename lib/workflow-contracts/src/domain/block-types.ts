...
export const BLOCK_FAMILIES = [...PUBLIC_BLOCK_FAMILIES, 'Protected'] as const;
...
export type FiscalStage =
  'trigger' | 'source' | 'logic' | 'validation' | 'field' | 'output' | 'ai-agent';
...
export type BlockRunStatus = 'pending' | 'running' | 'success' | 'warning' | 'error' | 'skipped';

export function isPublicBlockFamily(value: unknown): value is BlockFamily {
  return typeof value === 'string' && (PUBLIC_BLOCK_FAMILIES as readonly string[]).includes(value);
}
