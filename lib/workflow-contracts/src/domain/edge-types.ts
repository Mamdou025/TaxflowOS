...
export type WorkflowEdgeHistoryEntry = {
  id: string;
  action: 'created' | 'updated' | 'approved' | 'rejected' | 'disabled' | 'migrated' | 'split';
  by: string;
  at: string;
  notes?: string;
};
...
export const WORKFLOW_RELATIONSHIP_LABELS: Record<WorkflowRelationshipType, string> = {
...
export type RelationshipTypesByFamilyPair = Partial<
  Record<BlockFamily, Partial<Record<BlockFamily, readonly WorkflowRelationshipType[]>>>
>;

export function isWorkflowRelationshipType(value: unknown): value is WorkflowRelationshipType {
  return (
    typeof value === 'string' &&
...
export function isGovernedOutputRelationshipType(
  ...
  return (GOVERNED_OUTPUT_RELATIONSHIP_TYPES as readonly WorkflowRelationshipType[]).includes(
    relationshipType,
  );
}
...
export function isCandidateOutputRelationshipType(
  ...
  return (CANDIDATE_OUTPUT_RELATIONSHIP_TYPES as readonly WorkflowRelationshipType[]).includes(
    relationshipType,
  );
}
...
export function isOutputMappingRelationshipType(
  ...
  return (OUTPUT_MAPPING_RELATIONSHIP_TYPES as readonly WorkflowRelationshipType[]).includes(
    relationshipType,
  );
}
