...
export function isKnownWorkflowRelationshipType(value: unknown): value is WorkflowRelationshipType {
  return (
    typeof value === 'string' &&
...
export function isWorkflowRelationshipAllowed({
  ...
  return getAllowedRelationshipTypesForFamilies(sourceFamily, targetFamily).includes(
    relationshipType,
  );
}
