...
import { z } from 'zod';
import { AGENT_TOOL_REGISTRY } from '@workspace/agent-runtime/registry';
...
export function createSearchDocumentsTool(documents: AgentLabDoc[]): ToolSet[string] {
  return tool({
    description:
    ...
    execute: async ({ query, maxResults }) => {
      ...
      return {
        ...
        note:
          matches.length === 0
            ? 'No passages matched those keywords — try different terms.'
            : undefined,
      };
    },
  });
