...
export const MODEL_OPTIONS: { id: string; label: string; via: ModelVia; blurb: string }[] = [
  // Sina (auto): don't pin a model — Sina reads the question and routes to the right tier.
  {
    ...
    via: 'auto',
    blurb:
      'Sina reads each question and routes it: Haiku 4.5 (quick reads/lookups), Sonnet 5 (Q&A/analysis), Opus 4.8 (tax logic & calculations). Uses the gateway (AI_GATEWAY_API_KEY).',
  },
...
export type ToolCategory =
  'template' | 'real' | 'retrieval' | 'workflow-template' | 'workflow-action';
...
export const TOOL_CATALOG: ToolInfo[] = [
  ...
  {
    id: 'callApi',
    label: 'Call any JSON API',
    category: 'real',
    real: true,
    desc: 'Give a public http(s) endpoint (and optionally a POST body) → returns the record count, field names and sample records. Private addresses are refused.',
  },
  {
    id: 'focusBlock',
    label: 'focusBlock',
