...
import { buildApiRequest, describeApiConnectors } from '@workspace/source-connectors/connectors';
import {
  extractAtPath,
  fetchJsonPayload,
  findRecordArray,
...
function defineTool<S extends z.ZodTypeAny>(
  ...
  return {
    ...
    run: async (rawArgs: unknown) => {
      ...
      if (!parsed.success)
        return { error: 'Invalid arguments for this tool.', issues: parsed.error.issues };
...
const searchCanadianTax = defineTool(
  ...
  z.object({
    query: z
      ...
      .min(1)
      .max(500)
      .describe('what to look up, e.g. "capital gains inclusion rate 2024"'),
  ...
  async ({ query, limit }) => {
    ...
    if (!apiKey) {
      ...
      return {
        ...
        instruction:
          'Answer from your own knowledge. Do NOT claim to have searched official sources or cite a page you did not read, and briefly tell the user you could not verify this against live CRA / canada.ca sources.',
      };
    }
    ...
    try {
      ...
      const results = items
        .filter(
          (r) =>
            typeof r.url === 'string' &&
            OFFICIAL_CA_TAX_HOSTS.some((h) => hostMatches(r.url as string, h)),
        )
        .map((r) => ({
          title: r.title ?? '',
          url: r.url as string,
      ...
      return {
        ...
        results,
        instruction:
          'Cite the source URL for each fact you use. If the results do not answer the question, say so plainly.',
        note:
          results.length === 0
            ? 'No results from official Canadian sources — try rephrasing the query.'
            : undefined,
      };
    } catch (error) {
...
const searchWeb = defineTool(
  ...
  z.object({
    query: z
      ...
      .min(1)
      .max(500)
      .describe('the web search query, e.g. "OECD Pillar Two effective date"'),
    ...
    if (!apiKey) {
      ...
      if (process.env.AI_GATEWAY_API_KEY) {
        try {
          ...
          return {
            error:
              error instanceof Error ? `Search failed: ${error.message}` : 'Web search failed.',
          };
        }
      }
      ...
      return {
        ...
        instruction:
          'Answer from your own knowledge and tell the user you could not check live web sources.',
      };
    }
    try {
      const res = await fetch('https://api.firecrawl.dev/v1/search', {
  ...
  async ({ url }) => {
    ...
    try {
      const res = await fetch(target, {
        headers: { 'user-agent': 'Mozilla/5.0 (SinaxeSina)' },
        redirect: 'follow',
      });
...
const estimateForeignIncomeTax = defineTool(
  ...
  async ({ grossIncome, currency, taxYear }) => {
    ...
    let fxSource: string;
    try {
      const fx = await fetchAnnualAverageExchangeRate({
    ...
    return {
      ...
      assumptions:
        'Illustrative combined federal+provincial rates: 12.2% up to $500k CAD, 26.5% above. Not tax advice.',
    };
  },
);
...
const callApi = defineTool(
  ...
  z.object({
    url: z
      ...
      .max(2000)
      .describe(
        'the full public http(s) URL to call, e.g. https://api.usaspending.gov/api/v2/search/spending_by_award/',
      ),
    ...
    headers: z
      ...
      .optional()
      .describe('extra request headers, e.g. an Authorization header'),
    resultsPath: z
      .string()
      .optional()
      .describe(
        'dotted path to the array of records in the response, e.g. "results" or "data.items". Omit to auto-detect.',
      ),
  }),
    ...
    if (body) {
      try {
        ...
        return {
          ...
          instruction:
            'Fix the JSON and call the tool again. Tell the user the body was malformed.',
        };
      }
    }
