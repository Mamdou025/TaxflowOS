...
export type TemplateConfig = {
  ...
  buildSnapshot: () => WorkflowDefinition;
...
export type RunDetail = {
  workpaperRows?: Record<string, unknown>[];
  findings?: string[];
...
export type CoreResult = {
  execution: LocalToolRunnerResult;
  definition: ReturnType<TemplateConfig['buildSnapshot']>;
...
export function runTemplateCore(
  ...
  const snapshot = structuredClone(config.buildSnapshot());
  ...
  for (const inp of config.editableInputs ?? []) {
    ...
    if (inp.block) {
      ...
      if (b) {
        const defaults = b.config?.inputDefaults as Record<string, number> | undefined;
        b.config =
          defaults && inp.block.configKey in defaults
            ? {
                ...b.config,
                inputDefaults: { ...defaults, [inp.block.configKey]: inputVals[inp.key] },
              }
            : { ...b.config, [inp.block.configKey]: inputVals[inp.key] };
      }
    } else {
  ...
  if (opts.overrides.length) {
    const mapper = snapshot.blocks.find((b) => b.id === config.mapperBlockId);
    if (mapper) {
      const existing = Array.isArray(mapper.config?.keywordRules)
        ? (mapper.config!.keywordRules as unknown[])
        : [];
      const added = opts.overrides.map((o, i) => ({
        ...
        target: o.categoryId,
        keywords: [o.rowLabel.toLowerCase()],
        confidence: 1,
      }));
  ...
  if (opts.elected != null && config.elect) {
    ...
    if (paramBlock)
      paramBlock.config = { ...paramBlock.config, [config.elect.paramKey]: opts.elected };
  }

  const run = execute(snapshot);
  const results = run.result.results;
  const outputOf = (id: string) =>
    (results.find((r) => r.blockId === id)?.output as Record<string, unknown> | undefined) ?? {};
  ...
  const mapped: MappedRow[] = rawMapped.map((r) => ({
  ...
  const buckets = config.bucketKeys
    .filter((k) => k in named)
    .map((k) => ({ key: k, value: named[k] }));
  ...
  const detail: RunDetail = {
    ...
    lines,
    summary,
    ...(config.structuredRecords
      ? {
          workpaperRows: (outputOf(config.summaryBlockId).rows as Record<string, unknown>[]) ?? [],
          findings: run.result.warnings,
        }
      : {}),
  };
  const status =
...
export function runTemplateLoop(
  config: TemplateConfig,
  state: RunState,
  execute: DefinitionExecutor,
): RunOutcome {
  ...
  if (
    !rows.length ||
    (!config.structuredRecords && rows.some((row) => !Number.isFinite(row.amount)))
  ) {
    return {
      detail: null,
      done: false,
      ...
      blocker: {
        kind: 'error',
        message:
          'Supply at least one source record with a valid numeric amount. Empty or invalid records cannot be replaced with sample data.',
      },
    };
  }
  ...
  const core = runTemplateCore(
    config,
    { rows, overrides: overrideRules, elected: state.elected ?? undefined, inputs: state.inputs },
    execute,
  );
  if (core.status === 'error') {
    return {
      ...
      blocker: {
        ...
        message:
          core.errors.join('\n') ||
          'Workflow calculation failed. Correct the inputs or workflow definition before continuing.',
      },
    };
  }
  ...
  const needsReview =
    config.defaultRouteUnmatched && unresolved.length
      ? {
          count: unresolved.length,
          rows: unresolved.map((u) => ({ rowId: u.rowId, label: u.label, amount: u.amount })),
        }
      : undefined;
  ...
  const headlineVal =
    core.summaryValues[config.headlineKey] ?? core.lineValues[config.headlineKey] ?? 0;
  if (!(config.headlineKey in core.summaryValues) && !(config.headlineKey in core.lineValues)) {
    return {
      detail: core.detail,
      done: false,
      activeStage: 2,
      blocker: { kind: 'error', message: `The calculation did not produce ${config.headlineKey}.` },
    };
  }
  if (!state.approved) {
    return {
      ...
      blocker: {
        kind: 'approval',
        message:
          'All inputs are resolved and computed. Each figure below shows how it was derived. Approve to finalize.',
...
function headlineLabel(config: TemplateConfig): string {
  return (
    config.summaryRules.find((r) => r.resultKey === config.headlineKey)?.label ?? config.headlineKey
  );
}

/** Legacy sample preview. Never make classification, election or approval
 * decisions on behalf of the user. The name is retained for older integrations. */
export function runToCompletion(config: TemplateConfig, execute: DefinitionExecutor): RunOutcome {
  return runTemplateLoop(
    config,
    { ...initialRunState(), uploaded: true, rows: config.sampleRows },
    execute,
  );
}
...
export function resolveBlocker(state: RunState, blocker: RunBlocker, choiceId?: string): RunState {
  if (blocker.kind === 'error') return state;
