...
test('Build runs in place; Run and Build retain one execution and the original blocks', async ({
  ...
  const blocks = (definition: typeof original) =>
    definition.blocks.map(({ id, label, family, subtype, config, position }) => ({
      id,
      label,
      family,
      subtype,
  ...
  const edges = (definition: typeof original) =>
  ...
  const inspection = page.getByRole('region', { name: 'Run verification in Build' });
  const verifySelectedRun = async () => {
    await expect(inspection).toContainText((await read()).runs.at(-1)!.result.record.execution.id);
  };
  await verifySelectedRun();
  await expect(inspection).toContainText('error');
  await page.getByRole('button', { name: 'Run', exact: true }).first().click();
  await expect(
    page.getByRole('button', { name: 'Save changes and preview', exact: true }),
  ).toBeVisible();
  ...
  const outputs = (run: (typeof first.runs)[number]) =>
    JSON.parse(
      JSON.stringify(
  ...
  await inspection.getByRole('button', { name: 'Edit current draft' }).click();
  ...
  expect(
    successful.runs[2].result.result.results.find((result) => result.output.calculatedResults)
