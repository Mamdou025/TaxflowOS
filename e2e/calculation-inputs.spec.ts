...
test('Compute exposes number entry beside the formula and allows expected source fields', async ({ page }) => {
  ...
  const saved = await page.evaluate(async () => {
    ...
    const compute = entry.draft.blocks.find(
      (block) => block.config.toolId === 'logic.calculation_engine',
    )!;
    const formulas = structuredClone(compute.config.formulas);
    return {
      formulas,
      id: entry.id,
      label: compute.label,
      term: formulas.at(-1).resultKey,
    };
  });

  await page.goto(`/w/${saved.id}`);
  await page.getByRole('button', { name: 'Build', exact: true }).click();
  await page.getByRole('button', { name: saved.label, exact: true }).click();
  const restoredTerm = page
    .getByRole('button')
    .filter({ has: page.getByText(saved.term, { exact: true }) });
  await expect(restoredTerm).toContainText('500 / 2');
  await restoredTerm.click();
  await expect(page.getByText(/· rawRows › 0 › rate/)).toBeVisible();
  const restored = await page.evaluate(async (id) => {
    const { readWorkflowLibrary } = await import('/src/features/workflows-hub/workflow-library.ts');
    return readWorkflowLibrary()[id].draft.blocks.find(
      (block) => block.config.toolId === 'logic.calculation_engine',
    )!.config.formulas;
  }, saved.id);
  expect(restored).toEqual(saved.formulas);
});
