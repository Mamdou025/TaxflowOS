...
const WORKFLOW_DOMAINS = __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$agents$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["AGENTS"].filter((a)=>a.workflow && EXPERTISE[a.workflow]).map((a)=>({
        id: a.id,
        name: a.name,
        role: a.role,
...
const BY_WORKFLOW = Object.fromEntries(WORKFLOW_DOMAINS.map((d)=>[
        d.workflowId,
        d
    ]));
function domainForWorkflow(workflowId) {
    return workflowId ? BY_WORKFLOW[workflowId] ?? null : null;
}
function selectDomain(route) {
    if (route.target.kind === 'workflow' && route.target.id) {
...
"[project]/features/assistant/runtime/agents/sina.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
;
const SINA_NAME = 'Sina';
function sinaTurnDirective(route) {
    const domain = (0, __TURBOPACK__imported__module__$5b$project$5d2f$features$2f$assistant$2f$runtime$2f$agents$2f$specialists$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["selectDomain"])(route);
    if (!domain) {
        return null;
    }
    return `DOMAIN FOCUS FOR THIS TURN — You are ${SINA_NAME}, the workspace's tax specialist, and this turn is about ${domain.role.toLowerCase()}. ` + `Apply this domain expertise: ${domain.expertise} ` + `Answer with that expertise. Never fabricate figures — ground them in the provided context and use the deterministic workflow/calculation tools for official numbers. ` + `If the user shifts to another area, apply that domain's expertise or return to general assistance.`;
}
}),
...
var __TURBOPACK__imported__module__$5b$project$5d2f$features$2f$assistant$2f$runtime$2f$agents$2f$sina$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/features/assistant/runtime/agents/sina.ts [app-route] (ecmascript)");
...
const POST = async (req)=>{
    ...
    agent.use((input, next)=>{
        ...
        if (runtimeConfig.specialists && route) {
            const directive = (0, __TURBOPACK__imported__module__$5b$project$5d2f$features$2f$assistant$2f$runtime$2f$agents$2f$sina$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["sinaTurnDirective"])(route);
            if (directive) {
                ...
                context = [
                    ...
                    {
                        value: directive,
                        description: 'Domain focus for this turn (Sina — one unified agent)'
                    }
                ];
            }
        }
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0a69ce5c._.js.map