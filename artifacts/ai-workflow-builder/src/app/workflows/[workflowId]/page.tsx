...
const WorkflowEditor = ({ params: _params }: WorkflowPageProps) => {
  ...
  return (
    <div className="flex h-dvh w-full flex-col overflow-hidden">
      ...
      {!isMobile && panelCollapsed && (
        <button
          className="-translate-y-1/2 pointer-events-auto absolute top-1/2 right-0 z-20 flex h-12 w-7 items-center justify-center rounded-l-full border border-r-0 bg-background shadow-sm transition-colors hover:bg-muted"
          onClick={() => {
            setIsPanelAnimating(true);
        ...
        >
          ...
          >
            ...
            {!(isDraggingResize || panelCollapsed) && (
              <button
                className="-translate-x-1/2 -translate-y-1/2 absolute top-1/2 left-0 flex h-12 w-7 items-center justify-center rounded-r-none rounded-l-full border bg-background opacity-0 shadow-sm transition-opacity hover:bg-muted group-hover:opacity-100"
                onClick={(e) => {
                  e.stopPropagation();
                  setIsPanelAnimating(true);
