...
import { currentWorkflowNameAtom, edgesAtom, executionLogsAtom, nodesAtom, selectedExecutionIdAtom, type WorkflowNode, type WorkflowEdge } from '@/shared/workflow-engine/state/workflow-store';
...
export function templateDefinition(id: string): WorkflowDefinition | null {
  return getWorkflowConfig(id.replace(/^pf-/, ''))?.buildSnapshot()
    ?? createPortfolioWorkflowById(id) ?? null;
}

// Project a recorded execution onto the existing canvas; never alter its graph or configuration.
export function executionCanvas(nodes: WorkflowNode[], edges: WorkflowEdge[], run?: LocalToolRunnerResult) {
  return {
    nodes: nodes.map(node => ({ ...node, data: { ...node.data, status: run?.blockStatuses[node.id] ?? 'idle' as const } })),
    edges: edges.map(edge => ({ ...edge, data: { ...edge.data, runStatus: run?.edgeStatuses[edge.id] ?? 'idle' as const, runStatusRunId: run?.record.execution.id } })),
  };
}

export function executionLogs(run?: LocalToolRunnerResult) {
  return Object.fromEntries((run?.record.logs ?? []).map(log => [log.nodeId, {
    nodeId: log.nodeId, nodeName: log.nodeName, nodeType: log.nodeType, status: log.status, output: log.output,
  }]));
}

// Both views use the Run tab's existing versioning and graph runner.
export const runWorkflowAtom = atom(null, (get, set, options: { id: string; fromBuild?: boolean; saveDraft?: boolean; version?: number | null }) => {
  try {
    ...
    if (options.fromBuild) {
      const canvasDraft = createWorkflowDefinitionFromCanvas({
        nodes: get(nodesAtom), edges: get(edgesAtom), name: get(currentWorkflowNameAtom), existing: definition ?? createBlankWorkflow(),
      });
      const baseline = definition && createWorkflowDefinitionFromCanvas({
        ...workflowDefinitionToCanvas(definition), name: definition.name, existing: definition,
      });
    ...
    const base = entry ? replaceDraft(entry, definition) : createWorkflow(nextId, options.id, { ...definition, name: `${definition.name} — My workflow` });
    const executed = executeSavedWorkflow(base, {
      saveDraft: options.saveDraft,
      version: options.saveDraft === false ? options.version : undefined,
      requestId: crypto.randomUUID(),
    }, undefined, workflowRunContext(options.fromBuild ? 'builder' : 'run'));
    ...
    if (result.result.status === 'error') toast.error(`Version ${version.number} failed. Review the execution errors.`);
    else if (result.result.status === 'warning') toast.warning(`Version ${version.number} calculated with review findings.`);
    else toast.success(`Version ${version.number} executed. Review block results in Build or Run.`);
