...
//
// Sina — the one unified assistant — gets a real face instead of "Si" initials: the
// SinaGlyph spark mark (see sina-mark.tsx). Every other actor keeps initials.
// ─────────────────────────────────────────────────────────────────────────────

import type { Coworker } from '@/lib/coworkers';
import { SINA } from '@/lib/coworkers';
import { SinaGlyph } from './sina-mark';
...
export function CoworkerAvatar({
  ...
  const isSina = coworker.id === SINA.id;
  return (
    <span style={{ position: 'relative', flexShrink: 0, display: 'inline-flex', opacity: dim ? 0.45 : 1 }}>
      {isSina ? (
        // Sina's neumorphic spark disc — self-contained + round, with its own soft
