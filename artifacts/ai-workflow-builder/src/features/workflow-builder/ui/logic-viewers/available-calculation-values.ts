...
export function availableCalculationValues(
  ...
  const values = new Map<string, { key: string; label: string; value: number | null }>();
  ...
  const visit = (id: string) => {
    for (const edge of edges.filter(
      (edge) => edge.target === id && edge.data?.workflowEdge?.status !== 'disabled',
    )) {
      ...
      if (config.sourceKind === 'fapi_inputs') {
        for (const [key, value] of Object.entries(config)) {
          if (typeof value === 'number' && !['sourceVersion', 'fapiYear'].includes(key))
            add(key, value);
        }
      }
