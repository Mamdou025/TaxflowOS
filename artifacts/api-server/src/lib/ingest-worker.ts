...
async function runJob(job: ClaimedJob): Promise<void> {
  const { id, documentId, workspaceId, attempts, maxAttempts } = job;
  ...
  try {
    const result = await ingestDocument(workspaceId, documentId);

    if (result.ok) {
      await markJobDone(id);
      jobLog.info({ chunks: result.chunks }, 'Document ingested');
      return;
    }

    if (result.retryable && attempts < maxAttempts) {
      await requeueJob(id, attempts, result.error);
      jobLog.warn({ error: result.error }, 'Ingest retry scheduled (transient failure)');
      return;
    }
    ...
    jobLog.error({ error: result.error }, 'Ingest failed');
  } catch (err) {
    ...
    if (attempts < maxAttempts) {
      ...
      jobLog.error({ err }, 'Ingest job threw; will retry');
    } else {
      ...
      jobLog.error({ err }, 'Ingest job threw; retries exhausted');
    }
  }
}
