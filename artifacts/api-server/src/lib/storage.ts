...
// storage key (@workspace/db → documents). This module brokers access with the
// SERVICE-ROLE key (bypasses RLS) — it must never reach the browser.
//
//   • createSignedUploadUrl(key)  → a direct browser→Storage upload URL. Large
//     files skip the API entirely (never transit this server).
...
//
// FAIL-SOFT: if SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY are unset,
// isStorageConfigured() is false and the mutating helpers throw a clear
// StorageNotConfiguredError the routes turn into a 503 — the rest of the app is
// unaffected. Ported from the web app's platform/storage/supabase.ts.
// ─────────────────────────────────────────────────────────────────────────────
...
/** Remove an object (best-effort — a missing object is not an error upstream). */
