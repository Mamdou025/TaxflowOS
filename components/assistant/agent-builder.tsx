...
export function AgentBuilder() {
  ...
  return (
    <div className="h-full overflow-auto" style={{ background: '#18181c' }}>
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '32px 28px 48px', fontFamily: "'Inter', ui-sans-serif, system-ui, sans-serif" }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
          <span style={{ width: 34, height: 34, borderRadius: 10, display: 'grid', placeItems: 'center', background: 'rgba(168,146,214,0.16)', color: '#c4b5fd' }}><Bot size={18} /></span>
          <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: '#f4f4f6', letterSpacing: '-0.02em' }}>Agent Builder</h1>
        </div>
        <p style={{ margin: '0 0 24px', fontSize: 13.5, color: 'rgba(255,255,255,0.5)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {AGENTS.map((a) => (
            <div key={a.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '14px 16px', borderRadius: 14, background: '#25252f', border: '1px solid rgba(255,255,255,0.09)' }}>
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontSize: 14, fontWeight: 650, color: '#f4f4f6' }}>{a.name}</span>
                  <span style={{ fontSize: 10.5, fontWeight: 600, color: a.live ? '#7cc3a6' : 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{a.live ? 'Live' : 'Soon'}</span>
                </div>
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.55)', marginTop: 2 }}>{a.role}</div>
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', marginTop: 6, lineHeight: 1.45 }}>{a.tagline}</div>
              </div>
            </div>
          ))}
          ...
          <button
            disabled
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '14px 16px', borderRadius: 14, background: 'transparent', border: '1px dashed rgba(255,255,255,0.16)', color: 'rgba(255,255,255,0.4)', fontSize: 13, fontWeight: 600, cursor: 'not-allowed' }}
          >
